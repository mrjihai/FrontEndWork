...
module.exports = function (app) {
	...
	//feature- keyfeature
	app.get('/keyfeature',function(req,res){
		res.render('keyfeature')
	})
	//feature- performance
	...
	app.post('/login', function (req, res) {
		...
		User.get(name, function (err, user) {
			if (!user) {
				return res.json({ code: '0', data: "Account Not Existed!" });
			}
			if (user.password != password) {
				return res.json({ code: '0', data: "Wrong Password!" });
			}
	...
	app.post('/reg', function (req, res) {
		...
		User.get(newUser.name, function (err, user) {
			...
			if (user) {
				return res.json({ code: "0", data: 'Account has already existed' });
			}
			//如果不存在则新增用户
			newUser.save(function (err, user) {
				...
				res.json({ code: "1", data: 'Register Success!' });//code为1：成功
			});
		});
	});
	//发表
	...
	app.get('/remove/:name/:day/:title', function (req, res) {
		...
		Post.remove(currentUser.name, req.params.day, req.params.title, function (err) {
			...
			res.json({ code: "1", data: "delete success!" });
		});
	});
	//查詢搜索
	app.get('/search', function (req, res) {
		Post.search(req.query.keyword, function (err, posts) {
			...
			res.render('search', {
				title: "search results  - " + req.query.keyword,
				posts: posts,
				url: 'search',
				user: req.session.user,
			});
